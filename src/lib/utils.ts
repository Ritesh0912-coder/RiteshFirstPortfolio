...
export const SPACE_PLACEHOLDERS = [
    "https://images.unsplash.com/photo-1451187580459-43490279c0fa?q=80&w=1000&auto=format&fit=crop", // Earth
...
// Cloudinary Source (if available) - h7ZnhOKF8PXgHvV1t4V_cgUJbpE might be cloud name
const CLOUDINARY_CLOUD = "h7ZnhOKF8PXgHvV1t4V_cgUJbpE";
const CLOUDINARY_PLACEHOLDERS = [
    `https://res.cloudinary.com/${CLOUDINARY_CLOUD}/image/upload/v1/space/1`,
    `https://res.cloudinary.com/${CLOUDINARY_CLOUD}/image/upload/v1/space/2`,
    `https://res.cloudinary.com/${CLOUDINARY_CLOUD}/image/upload/v1/space/3`,
];
...
export function getRandomSpaceImage(seed: string = "") {
    if (!seed) return SPACE_PLACEHOLDERS[Math.floor(Math.random() * SPACE_PLACEHOLDERS.length)];

    // Hash the seed to get a consistent index
    let hash = 0;
    for (let i = 0; i < seed.length; i++) {
        hash = seed.charCodeAt(i) + ((hash << 5) - hash);
    }
    const index = Math.abs(hash) % SPACE_PLACEHOLDERS.length;
    return SPACE_PLACEHOLDERS[index];
}
...
export function getValidImageUrl(url: string | null | undefined): string | null {
    ...
    // 3. NUCLEAR BLACKLIST: Block ALL Google hosted images and specific unwanted assets
    // This is necessary to prevent the generic "GE" logo or unwanted manual overrides from appearing.
    // We block 'google' (covers google.com, googleusercontent.com, news.google.com)
    // and 'gstatic' (covers the logo hosting).
    // ADDITION: Broad Deadpool blocking and specific Unsplash ID
    if (
        lowerUrl.includes("google") ||
        lowerUrl.includes("gstatic") ||
        lowerUrl.includes("1608889335941-32ac5f2041b9") ||
        lowerUrl.includes("deadpool")
    ) {
        return null;
    }
...
export function getCompulsoryImage(url: string | null | undefined, contextText: string = ""): string {
    ...
    return getRandomSpaceImage(contextText || url || "");
}
